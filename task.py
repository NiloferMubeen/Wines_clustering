...
class matrix: 
            ...
            def get_distance(self,other_matrix,row_i):
                    ...
                    distance_matrix = []
                    # calculating the euclidean distance between data matrix and centroids (other matrix)
                    for x in other_matrix:
                            distance = np.sum((x - y)**2)
                            distance_matrix.append(distance)
                    dist =  np.array(distance_matrix).reshape(-1,1)  #reshaping it to (n,1); n -> rows in centroids
                    return matrix(dist) # returning the output as an object of the class matrix
            ...
            def get_weighted_distance(self,other_matrix,weights,row_i):
                ...
                for x in other_matrix:
                        distance = np.sum(weights * ((x - y)**2)) #here weights refer to the weight matrix
                        distance_matrix.append(distance)
...
def get_initial_weights(m): # m -> number of columns in data matrix
    ...
    return initial_weights   # this is the initial weight matrix
...
def get_centroids(data_matrix,S,K):
    ...
    for k in range(K):
        ...
        centroids[k] = np.mean(data_matrix.array_2d[indices[0]],axis=0) #updating the centroids
    return centroids
...
def get_separation_within(data_matrix,S,centroids,K):
    ...
    a_j = np.zeros((1, m))  #initializing an empty matrix of shape(1,m) m -> number of columns in data_matrix
    for k in range(K):
        cluster_indices = np.where(S == k)[0] # getting the indices where the elements of S matrix is equal to k
        for j in range(m):
            sum_distances = np.zeros((1,m)) # sum of the distances of every data data point and its cluster
            for i in cluster_indices:
                    distance = data_matrix.get_distance(centroids[k],i) #calculating the distance between the clusters and their corresponding data points(i)
                    sum_distances += np.sum(distance.array_2d.reshape(1,m),axis=0)
        a_j += sum_distances # sum of all the cluster distances
    return a_j
...
def get_separation_between(data_matrix,S,centroids,K):
    m = data_matrix.array_2d.shape[1]
    b_j = np.zeros((1, m)) 
    overall_means = matrix(np.mean(data_matrix.array_2d,axis=0)) # gives a matrix of dimension(1,13) as it calculates the mean of of all the columns
    for k in range(K):
        cluster_indices = np.where(S == k)
        n_k = len(cluster_indices)
        distance = overall_means.get_distance(centroids[k],0).array_2d.reshape(1,m) # here row is 0 as overall means has only one row
        b_j += (n_k * distance)
    return b_j
...
def get_groups(data_matrix,K):
    ...
    while True:
            ...
            for row_i in range(n):
                    ...
                    if np.array_equal(S, new_S):  #checks if the new and the old S matrix are the same after the updation
                        break
                    S = new_S.copy()
...
def get_new_weights(data_matrix,centroids,old_weight_vector,S,K):
    ...
    new_weights = np.zeros_like(old_weight_vector) 
    # Loop over each feature j to calculate the new weight
    for j in range(m):
        b_a = b_j[0, j] / a_j[0, j] 
        new_weights[0, j] = 0.5 * (old_weight_vector[0, j] + (b_a / np.sum(b_j/a_j)))
    return new_weights
